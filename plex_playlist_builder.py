...
class PlexPlaylistBuilder():
	def __init__(self, username=None, password=None, resource=None):
		plex_library = PlexConnection(username=username, password=password, resource=resource).music_library
		self.music_library = PlexMusic(plex_library)

		self._popular_tracks = None
		self._recent_tracks = None
		self._spice_tracks = None

		self._play_count_min = None
		self._play_count_max = None
		self._last_played_min = None
		self._last_played_max = None
		self._artist_play_count_min = None
		self._artist_play_count_max = None
		self._album_play_count_min = None
		self._album_play_count_max = None
		self._genre_play_count_min = None
		self._genre_play_count_max = None
		self._genre_play_counts = None
		self._genre_song_counts = None
	...
	def build_playlist(self, playlist_title=None, track_count=50):
		...
		playlist_slots = [popular_picker, recent_picker, popular_picker, spice_picker, popular_picker, recent_picker]
	...
	def artist_play_count_min(self):
		if self._artist_play_count_min is None:
			print('Getting artist_play_count_min')
	...
	def artist_play_count_max(self):
		if not self._artist_play_count_max:
			print('Getting artist_play_count_max')
	...
	def album_play_count_min(self):
		if self._album_play_count_min is None:
			print('Getting album_play_count_min')
	...
	def album_play_count_max(self):
		if not self._album_play_count_max:
			print('Getting _album_play_count_max')
	...
	def genre_play_count_min(self):
		if self._genre_play_count_min is None:
			print('Getting _genre_play_count_min')
			self._build_mix_max_values()
		if self._genre_play_count_min is None:
	...
	def genre_play_count_max(self):
		if not self._genre_play_count_max:
			print('Getting _genre_play_count_max')
			self._build_mix_max_values()
	...
	def genre_play_counts(self):
		if not self._genre_play_counts:
			print('Getting _genre_play_counts')
			self._build_mix_max_values()
	...
	def genre_song_counts(self):
		if not self._genre_song_counts:
			print('Getting _genre_song_counts')
			self._build_mix_max_values()
	...
	def _build_mix_max_values(self):
		...
		for artist in self.music_library.all_artists():
			...
			for genre in artist.styles:
				genre_id = genre.id
				genre_play_count = genre_play_counts[genre_id]
				genre_play_count += artist_play_count
				genre_play_counts[genre_id] = genre_play_count

				genre_song_count = genre_song_counts[genre_id]
				genre_song_count += artist_track_count
				genre_song_counts[genre_id] = genre_song_count

		min_genre_plays = 10000000
		max_genre_plays = 0
		for genre in genre_play_counts:
			play_count = genre_play_counts[genre]
			if play_count > max_genre_plays:
				max_genre_plays = play_count
			if play_count < min_genre_plays:
				min_genre_plays = play_count
	...
	def _build_track_dict(self, plex_track, category):
		...
		if plex_track.userRating:
			rating_normalized = self._normalize(
				plex_track.userRating, max_value=10, min_value=1
			)
			if rating_normalized > 0:
				rating_normalized_to_apply = rating_normalized / 2

		last_played_adjusted = 0
		track_last_played = plex_track.lastViewedAt
		if track_last_played:
			last_played_normalized = self._normalize(
				track_last_played.timestamp(),
		...
		artist_play_count_normalized = self._normalize(
			track_artist.viewCount,
		...
		for track_genre in track_artist.styles:
		...
		weight = 1 + play_count_normalized + rating_normalized_to_apply + last_played_adjusted + artist_play_count_normalized + (genre_play_count_normalized / 2)
		weight = weight * 1000
